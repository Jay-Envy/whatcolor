...
namespace WhatColor.Helpers
{
    public class ColorTransformer
    {
        ...
        //public int CMYK { get => CMYK; set => GetCMYK(RGB); }
        //public int RGB { get => RGB; set => RGB = Convert.ToInt32(HEX, 16); }
        //public string Name { get; set; }
        //public string TrendingUrl { get => TrendingUrl; set => TrendingUrl = $"https://www.coolors.co/trending/{HEX}"; }
        //public string ComplementaryHex { get => ComplementaryHex; set => GetComplementaryHex(RGB); }

        //Get CMYK (credit: https://www.cyotek.com/blog/converting-colours-between-rgb-and-cmyk-in-csharp)
        public static string GetCMYK(string RGB)
        {
            double c, m, y, k;
            ...
            double r = Convert.ToDouble(RGB.Substring(0, 3));
            double g = Convert.ToDouble(RGB.Substring(2, 3));
            double b = Convert.ToDouble(RGB.Substring(5, 3));
        ...
        ==/--------------------------------------------/==
        */
        public static string GetComplementaryHex(string RGB)
        {
            if (RGB.Length < 9)
            {
        ...
        private static double H2RGB(double var1, double var2, double h)
        {
            if (h < 0)
            {
                h += 1;
